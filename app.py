...
import random # Aggiunto per i dati demo
import folium
import pandas as pd
import requests
...
def init_honeycomb_state(field_name, crop="Oliveto"):
    """
    FIX: Associa lo stato dell'alveare al nome del campo, così ogni campo
    ha la sua visualizzazione indipendente e i dati non si sovrappongono per tutti.
    """
    risk_mapping = {
    ...
    if "honeycomb_data" not in st.session_state:
        st.session_state.honeycomb_data = {}

    if field_name not in st.session_state.honeycomb_data:
        st.session_state.honeycomb_data[field_name] = [
    ...
    st.session_state.honeycomb_cells = st.session_state.honeycomb_data[field_name]
...
def fetch_weather_advanced(lat, lon):
    # FIX: Rimosso "models=icon_seamless" che spesso faceva fallire la chiamata meteo.
    url = (
...
# FIX: Generatore di Dati Falsi per salvare la Demo se Copernicus fallisce
def generate_mock_satellite_data():
    ...
    for i in range(12):
        ...
        mock_data["data"].append({
            ...
            "outputs": {
                "ndvi": {"bands": {"B0": {"stats": {"mean": round(random.uniform(0.55, 0.85), 3)}}}},
                "msavi": {"bands": {"B0": {"stats": {"mean": round(random.uniform(0.40, 0.70), 3)}}}},
                "ndmi": {"bands": {"B0": {"stats": {"mean": round(random.uniform(0.30, 0.60), 3)}}}}
            }
        })
    return mock_data
...
def fetch_satellite_statistics(lat, lon):
    ...
    try:
        auth_res = requests.post(
            ...
            timeout=5, # Timeout breve per non bloccare la demo
        )
    ...
    # SE L'API FALLISCE, RITORNA I DATI MOCK DELLA DEMO
...
init_honeycomb_state(st.session_state.active_field_name, st.session_state.active_crop)
...
if not fields_df.empty:
    ...
    if selected_field != st.session_state.active_field_name:
        ...
        # Inizializza/Carica i dati specifici del nuovo campo
        init_honeycomb_state(row["name"], row["crop"])
        st.rerun()
...
if num_campi < MAX_CAMPI_ABBONAMENTO:
    ...
    if st.sidebar.button("💾 Salva Campo"):
        if new_name:
            ...
            init_honeycomb_state(new_name, new_crop)
...
col3.metric("🌧️ Pioggia 7gg", f"{rain_sum:.1f} mm")
